<script lang="ts">
  ...
  import { IssueCode } from '#/def/export-endpoints';
  ...
  <div class="w-300 pb-100">
    <form use:form class="mb-20">
      ...
      <div class="flex justify-end mb-10">
        <a href="/login" class="text-12 text-primary hover:opacity-75">{$t('login.back_to_login')}</a>
      </div>
      <RegisterButton disabled={!$isValid || isSubmitting} loading={isSubmitting}>{$t('login.send_code')}</RegisterButton>
    </form>
  </div>
</div>
